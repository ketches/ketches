...
function onSwitchEnv(envID: string) {
    ...
    router.push({ name: 'env-page', params: { id: envID } })
}
</script>
    ...
    <BreadcrumbItem v-if="userStore.getCurrentEnvRefs.length > 0" @mouseenter="envHover = true"
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
                <DropdownMenuItem v-if="activeEnvRef" :key="activeEnvRef.envID" disabled>
                ...
                <DropdownMenuItem
                    ...
                    <RouterLink :to="{ name: 'env-page', params: { id: envRef.envID } }" v-slot="{ navigate, href }"
                        class="flex items-center gap-2 w-full">
                        <div class="h-4 w-4" />
