...
const profileFormSchema = toTypedSchema(
  z.object({
    ...
    requestCPU: z.number().min(0),
    requestMemory: z.number().min(0),
    limitCPU: z.number().min(100, {
      message: "CPU 配额不能小于 0.1 核。",
    }),
    limitMemory: z.number().min(128, {
...
const onSubmit = handleSubmit(async (values) => {
  if (
    values.replicas === app.value.replicas &&
...
<template>
  <div>
    <h3 class="text-lg font-medium">资源配置</h3>
