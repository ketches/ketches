...
const columns: ColumnDef<appGatewayModel>[] = [
    ...
    },
    {
        ...
        header: "访问地址",
        cell: ({ row }) => {
            if (row.original.protocol === 'http' || row.original.protocol === 'https') {
                return h('div', { class: row.original.exposed ? '' : 'text-muted-foreground' }, `${row.original.domain ? row.original.domain + row.original?.path
                    || '' : "-"}`)
            } else if (row.original.protocol === 'tcp' || row.original.protocol === 'udp') {
                return h('div', { class: row.original.exposed ? '' : 'text-muted-foreground' }, `0.0.0.0:${row.original.gatewayPort}`)
            }
        },
    },
    {
        ...
        cell: ({ row }) => {
            return h('div', { class: "flex justify-end mr-2 gap-2" }, [
                ...
                h(Button, {
                    variant: 'outline',
                    onClick: async () => {
                        await deleteAppGateway(row.original.appID, [row.original.gatewayID])
                        listData.value = listData.value.filter(item => item.gatewayID !== row.original.gatewayID)
...
<template>
    ...
    <div class="flex flex-col flex-grow">
        <div class="flex gap-2 items-center pb-4 w-full">
            <Input class="max-w-sm" placeholder="搜索网关"
                :model-value="table.getColumn('accessAddress')?.getFilterValue() as string"
                @update:model-value=" table.getColumn('accessAddress')?.setFilterValue($event)" />
