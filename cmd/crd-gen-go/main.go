...
// Ref: https://github.dev/vmware-tanzu/velero
package main
...
// This is relative to root of the project
...
func CRDs() []*apiextv1.CustomResourceDefinition {
	...
	decode := scheme.Codecm.UniversalDeserializer().Decode
...
func main() {
	headerBytes, err := os.ReadFile(boilerplateFile)
	if err != nil {
		log.Fatalln(err)
	}
	...
	manifests, err := os.ReadDir("../../config/crd/bases")
	if err != nil {
		log.Fatalln(err)
	}

	for _, crd := range manifests {
		file, err := os.Open("../../config/crd/bases/" + crd.Name())
		if err != nil {
			log.Fatalln(err)
		}
	...
	t, err := template.New("crd").Parse(tmpl)
	if err != nil {
		log.Fatalln(err)
	}
