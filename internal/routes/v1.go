	...
	v1 := r.Group("/api/v1")
	{
		...
		authorized.Use(middlewares.Auth())
		{
			...
			codeRepos := authorized.Group("/code-repositories")
			{
				...
				codeReposRead.GET("/:repoID/build-configs", handlers.ListCodeRepositoryBuildConfigs)
				codeReposRead.GET("/:repoID/build-configs/:configID", handlers.GetCodeRepositoryBuildConfig)
			...
			apps := authorized.Group("/apps")
			{
				...
				appsRead.GET("/:appID/build-config", handlers.GetBuildConfig)
				appsRead.GET("/:appID/build-config/container-registries", handlers.ListAvailableContainerRegistries)
