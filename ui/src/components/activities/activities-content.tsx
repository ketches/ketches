...
export function ActivitiesContent({ embedded = false, scope = "auto", scopedUserId: scopedUserIdProp }: ActivitiesContentProps) {
  const authUser = useAuthStore((state) => state.user)
  const authRole = authUser?.role
  const isAdmin = scope === "personal" ? false : authRole === "admin"
  const scopedUserId = scope === "personal" ? (scopedUserIdProp ?? authUser?.id) : undefined
  ...
  const { data: response, isLoading, isFetching } = useQuery({
    ...
    queryFn: () => {
      const params = {
        user_id: scopedUserId || undefined,
    ...
    enabled: scope === "personal" ? Boolean(scopedUserId) : true,
  ...
  const columns = React.useMemo<ColumnDef<OperationLogItem>[]>(
    () => [
        ...
        : []),
      {
        accessorKey: "action",
        header: "Action",
        cell: ({ row }) => <span>{toTitleCase(row.original.action)}</span>,
      },
      {
