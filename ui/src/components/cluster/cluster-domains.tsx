...
export function ClusterDomains({ clusterId }: ClusterDomainsProps) {
  ...
  const { data: response, isLoading } = useQuery({
    queryKey: ["cluster-domains", clusterId],
    queryFn: () => domainsApi.listByCluster(clusterId),
  })
  const items = response?.items ?? []

  const createMutation = useMutation({
    mutationFn: (data: DomainFormData) => domainsApi.createForCluster(clusterId, {
      ...data,
      domain: normalizeDomainValue(data.domain),
    }),
    onSuccess: () => {
      toast.success("Domain added")
      queryClient.invalidateQueries({ queryKey: ["cluster-domains", clusterId] })
      handleCloseDialog()
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: DomainFormData }) => domainsApi.update("cluster", clusterId, id, {
      name: data.name,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => domainsApi.delete("cluster", clusterId, id),
    onSuccess: () => {
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Cluster-level domains available to applications in this cluster.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable
