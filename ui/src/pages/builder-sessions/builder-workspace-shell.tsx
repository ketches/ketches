...
export function BuilderWorkspaceShell() {
  ...
  const projectRole = useProjectRole(workspace.projectId)
  const isViewer = projectRole === "viewer"
  const [createEnvironmentOpen, setCreateEnvironmentOpen] = React.useState(false)
  const { data: envsResponse, isLoading: isEnvsLoading } = useQuery({
    queryKey: ["builder-all-envs", workspace.projectId],
    queryFn: () => envsApi.list(workspace.projectId!, { page: 1, page_size: 100 }),
